...
using dependAnt.Core.Monitoring;

namespace dependAnt.Core.Extensions
{
    public static class ObservableExtensions
    {
        ...
        public static IObservable<T> CacheFirstResult<T>(this IObservable<T> observable)
        {
            ...
            return observable
                .Take(1)
                .PublishLast()
                .LazilyConnect(new SingleAssignmentDisposable());
        }
        ...
        /// Anytime the stream OnNext happens twice below minimumUpdatePeriod, the second update gets delayed to respect the minimumUpdatePeriod.
        /// If more than 2 updates happen, only the last update is pushed.
        ...
        /// <typeparam name="T">Update type</typeparam>
        /// <param name="source">Source stream</param>
        /// <param name="stalenessPeriod">If source steam does not OnNext any update during this period, it is declared stale</param>
        /// <param name="scheduler"></param>
        /// <returns></returns>
        ...
        public static IObservable<TProp> ObserveProperty<TSource, TProp>(
            ...
            bool observeInitialValue) where TSource : INotifyPropertyChanged
        {
            return Observable.Create<TProp>(o =>
            {
                ...
                var observable = 
                    from evt 
                    in Observable.FromEvent<PropertyChangedEventHandler, PropertyChangedEventArgs>(
                        h => (s, e) => h(e),
                        h => source.PropertyChanged += h,
                        h => source.PropertyChanged -= h)
        ...
        public static IObservable<TProp> ObserveProperty<TSource, TProp>(
            this TSource source,
            Expression<Func<TSource, TProp>> propertyExpression) where TSource : INotifyPropertyChanged
        {
